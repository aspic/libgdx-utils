...
public class List<T> extends Widget implements Cullable {
	...
	public List (Array items, Skin skin) {
		this(items, skin.get(ListStyle.class));
	}

	public List (Array items, Skin skin, String styleName) {
		this(items, skin.get(styleName, ListStyle.class));
	}

	public List (Array items, ListStyle style) {
		setStyle(style);
		setItems(items);
	...
	public void draw (SpriteBatch batch, float parentAlpha) {
		...
		for (int i = 0; i < items.size; i++) {
			if (cullingArea == null || (itemY - itemHeight <= cullingArea.y + cullingArea.height && itemY >= cullingArea.y)) {
				...
				font.draw(batch, items.get(i).toString(), x + textOffsetX, y + itemY - textOffsetY);
				if (selectedIndex == i) {
	...
	public void setSelectedIndex (int index) {
		if (index < 0 || index >= items.size)
			throw new GdxRuntimeException("index must be >= 0 and < " + items.size + ": " + index);
		selectedIndex = index;
	}
